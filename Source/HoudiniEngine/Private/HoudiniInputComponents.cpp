...
static TSharedPtr<FHoudiniStaticMeshComponentInput> FindOrCreateMeshComponentInput(TArray<TSharedPtr<FHoudiniComponentInput>>& InOutComponentInputs,
	const FHoudiniStaticMeshComponentInput::EHoudiniSettingsNodeType& TargetSettingsNodeType)
{
	for (int32 HolderIdx = InOutComponentInputs.Num() - 1; HolderIdx >= 0; --HolderIdx)
	{
		const TSharedPtr<FHoudiniStaticMeshComponentInput>& OldComponentInput = StaticCastSharedPtr<FHoudiniStaticMeshComponentInput>(InOutComponentInputs[HolderIdx]);
	...
	return MakeShared<FHoudiniStaticMeshComponentInput>(TargetSettingsNodeType);
}

static TPair<const UStaticMesh*, TArray<const UMaterialInterface*>> GetStaticMeshInputIdentifier(
	const UStaticMesh* SM, const UStaticMeshComponent* SMC, bool& bOutHasVertexColor)
{
	bOutHasVertexColor = false;
	for (const FStaticMeshComponentLODInfo& LODInfo : SMC->LODData)
	{
		if (LODInfo.OverrideVertexColors && LODInfo.OverrideVertexColors->GetNumVertices())
		{
			bOutHasVertexColor = true;
			return TPair<const UStaticMesh*, TArray<const UMaterialInterface*>>(nullptr, TArray<const UMaterialInterface*>{});
		}
	}
	...
	for (const FStaticMaterial& StaticMaterial : SM->GetStaticMaterials())
	{
		...
		if (IsValid(SMC))
		{
			UMaterialInterface* OverrideMaterialInterface = SMC->GetMaterial(SMC->GetMaterialIndex(StaticMaterial.ImportedMaterialSlotName));
			if (IsValid(OverrideMaterialInterface))
				Material = OverrideMaterialInterface;
		}
		else
		{
			if (IsValid(StaticMaterial.MaterialInterface))
				Material = StaticMaterial.MaterialInterface;
		}

		Materials.Add(Material);
	}

	return TPair<const UStaticMesh*, TArray<const UMaterialInterface*>>(SM, Materials);
}
	...
	TMap<TPair<const UStaticMesh*, TArray<const UMaterialInterface*>>, TPair<int32, TArray<FInt32Interval>>>& InOutMeshPatternMap)
{
	...
	const TPair<const UStaticMesh*, TArray<const UMaterialInterface*>> MeshIdentifier = GetStaticMeshInputIdentifier(SM, SMC, bHasVertexColor);
	if (!bHasVertexColor)
	{
	...
	TSharedPtr<FHoudiniStaticMeshComponentInput> MeshComponentInput = FindOrCreateMeshComponentInput(InOutOldComponentInputs,
	...
	if (bHasVertexColor)  // Means this is a unique mesh
		HOUDINI_FAIL_RETURN(FHoudiniSopCopyToPoints::HapiSetSourcePointGroup(SettingsNodeId,
	...
	int32& InOutInstancerNodeId, TArray<TSharedPtr<FHoudiniComponentInput>>& InOutComponentInputs, TArray<FHoudiniComponentInputPoint>& InOutPoints)
{
		...
		Components[ComponentIndices[0]]->IsA<UStaticMeshComponent>())  // Maybe is just AStaticMeshActor
	{
		...
		if (IsValid(SM))
		{
			TSharedPtr<FHoudiniStaticMeshComponentInput> SMCInput = FindOrCreateMeshComponentInput(InOutComponentInputs,
	...
	for (const auto& MeshPattern : MeshPatternMap)
	{
		...
		HOUDINI_FAIL_RETURN(FHoudiniSopCopyToPoints::HapiSetSourcePointGroup(MeshPattern.Value.Key, Pattern));
	}
...
bool FHoudiniSkeletalMeshComponentInputBuilder::IsValidInput(const UActorComponent* Component)
{
	return Component->IsA<USkeletalMeshComponent>();
}

bool FHoudiniSkeletalMeshComponentInputBuilder::HapiUpload(UHoudiniInput* Input, const bool& bIsSingleComponent,
	const TArray<const UActorComponent*>& Components, const TArray<FTransform>& Transforms, const TArray<int32>& ComponentIndices,
	int32& InOutInstancerNodeId, TArray<TSharedPtr<FHoudiniComponentInput>>& InOutComponentInputs, TArray<FHoudiniComponentInputPoint>& InOutPoints)
{
	int32 NumSMCs = 0;
	for (const int32& CompIdx : ComponentIndices)
	{
		const USkeletalMeshComponent* SMC = Cast<USkeletalMeshComponent>(Components[CompIdx]);
		const USkeletalMesh* SM = SMC->GetSkeletalMeshAsset();
		if (!Input->GetSettings().bImportAsReference && IsValid(SM))
		{
			TSharedPtr<FHoudiniSkeletalMeshComponentInput> SKMCInput =
				StaticCastSharedPtr<FHoudiniSkeletalMeshComponentInput>(InOutComponentInputs.IsValidIndex(NumSMCs) ?
					InOutComponentInputs[NumSMCs] : InOutComponentInputs.Add_GetRef(MakeShared<FHoudiniSkeletalMeshComponentInput>()));

			int32& MeshNodeId = SKMCInput->MeshNodeId;
			int32& SkeletonNodeId = SKMCInput->SkeletonNodeId;
			HOUDINI_FAIL_RETURN(UHoudiniInputSkeletalMesh::HapiImport(SMC->GetSkeletalMeshAsset(), SMC, Input->GetSettings(), Input->GetGeoNodeId(),
				MeshNodeId, SKMCInput->MeshHandle, SkeletonNodeId, SKMCInput->SkeletonHandle));

			int32& SettingsNodeId = SKMCInput->SettingsNodeId;
			if (SettingsNodeId < 0)
			{
				HOUDINI_FAIL_RETURN(FHoudiniEngineSetupKineFXInput::HapiCreateNode(Input->GetGeoNodeId(),
					FString::Printf(TEXT("%s_%08X"), *SMC->GetName(), FPlatformTime::Cycles()), SettingsNodeId));
			...
			if (Actor)
				SettingsInput.AppendAttribute(HAPI_ATTRIB_UNREAL_ACTOR_OUTLINER_PATH,
					EHoudiniAttributeOwner::Point, GetActorOutlinerPath(Actor));
			HOUDINI_FAIL_RETURN(SettingsInput.HapiUpload(SettingsNodeId, Transforms[CompIdx]));

			++NumSMCs;
		}
		else
		{
			FHoudiniComponentInputPoint InputPoint;
			InputPoint.bHasBeenCopied = false;
			InputPoint.AssetRef = TCHAR_TO_UTF8(*(IsValid(SM) ? FHoudiniEngineUtils::GetAssetReference(SM) : SMC->GetClass()->GetPathName()));
			InputPoint.Transform = Transforms[CompIdx];
			if (IsValid(SM))
			{
				const FBox Box = SM->GetBounds().GetBox();
				const FVector3f Min = FVector3f(Box.Min) * POSITION_SCALE_TO_HOUDINI_F;
				const FVector3f Max = FVector3f(Box.Max) * POSITION_SCALE_TO_HOUDINI_F;
				InputPoint.MetaData = TCHAR_TO_UTF8(*FString::Printf(TEXT("{\"bounds\":[%f,%f,%f,%f,%f,%f]}"), Min.X, Max.X, Min.Z, Max.Z, Min.Y, Max.Y));
			}
			InOutPoints.Add(InputPoint);
		}
	}

	for (int32 SMCIdx = InOutComponentInputs.Num() - 1; SMCIdx >= NumSMCs; --SMCIdx)
		HOUDINI_FAIL_RETURN(InOutComponentInputs.Pop()->HapiDestroy(Input));

	return true;
}

void FHoudiniSkeletalMeshComponentInputBuilder::AppendInfo(const TArray<const UActorComponent*>& Components, const TArray<FTransform>& Transforms,
	const TArray<int32>& ComponentIndices, const TSharedPtr<FJsonObject>& JsonObject)
{
	// TODO:
}
